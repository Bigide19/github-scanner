...
const Exporter = (() => {
  ...
  function toMarkdown(repos) {
    const header = '| Name | Description | Language | Updated | Created | Visibility | Permission | Archived |';
    const separator = '| --- | --- | --- | --- | --- | --- | --- | --- |';
    ...
    const rows = (repos || []).map((repo) => {
      ...
      return `| ${name} | ${description} | ${language} | ${updated} | ${created} | ${visibility} | ${permission} | ${archived} |`;
    });
  ...
  function toCSV(repos) {
    const BOM = '\uFEFF';
    const header = ['Name', 'Description', 'Language', 'Updated', 'Created', 'URL', 'Visibility', 'Permission', 'Archived', 'Stars'];
